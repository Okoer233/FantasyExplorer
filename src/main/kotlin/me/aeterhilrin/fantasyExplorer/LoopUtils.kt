...
import me.aeterhilrin.fantasyExplorer.PinyinUtils.transToCitizensYml
import me.aeterhilrin.fantasyExplorer.PinyinUtils.transToMMYml
...
class LoopUtils(private val plugin: FantasyExplorer) {
    ...
    fun villagerTradesToJson(playerName: String, villager: org.bukkit.entity.Villager): Boolean {
        return try {
            ...
            tradeData.transToCitizensYml(plugin)
            // 记录到JSON文件
    ...
    fun bookToJson(playerName: String, bookItem: ItemStack): Boolean {
        return try {
            ...
            //写入yml Book MythicMobs
            bookData.transToMMYml(plugin)
            // 写入JSON文件
    ...
    fun spawnerToJson(playerName: String, commandBlock: Block): Boolean {
        return try {
            ...
            // 解析刷怪笼数据
            val spawnerData = spawnerParser.parseCommand(command,commandBlock)
            if (spawnerData == null) {
                plugin.logger.warning("无法解析刷怪笼数据")
                return false
            }
            spawnerData.transToMMYml(plugin)
            // 记录到JSON文件
